...
class ValueFinder:
    ...
    def kelly_stake(self, model_prob, odds):
        b = odds - 1
        p = model_prob
        q = 1 - p
        if b <= 0:
            return 0.0
        k = (p * b - q) / b
        return max(0.0, min(k * self.kelly_frac, 0.1))

    def find_bets(self, predictions_df, odds_dict):
        """
        predictions_df: columns fixture_id, home_prob, draw_prob, away_prob, btts_prob
        odds_dict: dict fixture_id -> dict with keys 'home', 'draw', 'away', 'btts_yes'
        """
        bets = []
